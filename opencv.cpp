#include "opencv.hpp"
cv::Mat createOne(std::vector<cv::Mat> & images, int min_gap_size)
{
	/*
...
void printBoard() {
    for (int i=0; i<15; i++) {
        for(int j=0; j<15; j++)
            cout << string(1,GAME_STATE[j][i]) << " ";
        cout << endl;
    }
    cout <<endl<<endl<<endl<<endl;
}

Mat& ftrImage(Mat &img, RGB lowColor, RGB rep,int radius, bool allbut = false) {
...
int pixelCount(Mat &img, RGB color) {
    int count=0;
    for (int i =0; i <img.cols; i++)
        for(int j = 0; j < img.rows; j++) {
            Vec3b *intensity = &img.at<Vec3b>(j, i);
...
void cleanImage(Mat& img) {
    ...
    ftrImage(img,RGB(255,255,255),RGB(0,0,0),5); //replace shades of white with black letter color
    ftrImage(img,RGB(76,25,0),RGB(0,0,0),35);
    //Apply binary mask, all pixels not in the range of RGB(0,0,0) + 0..10 are converted to white
    ftrImage(img,RGB(0,0,0),RGB(255,255,255), 10, true);
}
...
int main(int argc, char* argv[])
{
    ...
    int y_letters = 1570; //y offset of the left corner of the letters panel
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if ((float)pixelCount(templ,RGB(255,255,255))/(templ.cols*templ.rows) > .95) {
            cout << '_';
            continue;
        }
        //   imshow("t",templ);
    ...
    for (int i = 0; i <15; i++)
        for (int j=0; j<15; j++) {
            ...
            float whiteDensity =((float)(pixelCount(templ,RGB(255,255,255))))/(bk*bk);
